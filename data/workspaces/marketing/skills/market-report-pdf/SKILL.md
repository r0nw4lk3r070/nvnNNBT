...
- Triggered by `report-pdf` or `report-pdf <domain>`
...
1. Recommend the user run `audit <url>` first for the best results
...
- Footer: "Generated by AI Marketing Suite for nanobot"
...
1. Run `audit <url>` -- Generates comprehensive audit data
2. Run `competitors <url>` -- Adds competitor comparison data
3. Run `seo <url>` -- Adds detailed SEO findings
4. Run `landing <url>` -- Adds CRO analysis
5. Run `report-pdf <url>` -- Compiles everything into a PDF
