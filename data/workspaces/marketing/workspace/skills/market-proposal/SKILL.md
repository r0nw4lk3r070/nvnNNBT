...
- Triggered by `proposal` or `proposal <client name>`
...
**If audit data exists:** Check for any previous `audit` results. If found, automatically incorporate the findings into the Situation Analysis section for a data-backed proposal.
...
Present your analysis of the client's current marketing. This is where audit data from `audit` is invaluable.
...
3. **Competitive Landscape** -- How they compare to competitors (from `competitors` if available)
