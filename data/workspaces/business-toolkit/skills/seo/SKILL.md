...
- `read_file` — read site files, configs, and existing content for audit
- `write_file` — create or update SEO assets (meta tags, schema, sitemaps)
- `edit_file` — revise existing pages with SEO improvements
- `web_search` — keyword research, competitor analysis, search trend data
- `web_fetch` — audit live pages, check competitor content, verify structured data
- `exec` — run site crawls, check redirects, validate sitemaps, run Lighthouse audits
...
Location: src/routes/products/[slug].py
...
- `brand-voice` for consistent writing style across SEO content
- `market-research` for competitive SEO analysis
- `content-engine` for creating SEO-optimized content