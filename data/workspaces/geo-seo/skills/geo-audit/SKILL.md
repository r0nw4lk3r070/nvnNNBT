...
1. Use web_fetch to retrieve the homepage at the provided URL.
...
## Output: Write in Stages

**CRITICAL: Do NOT attempt to write the entire report in one write_file call.** The report is too large for a single output and will be truncated or fail silently. Write it in 3 stages:

**File path**: `workspace/reports/GEO-AUDIT-REPORT-[domain].md`

### Stage 1: Header + Summary + Scores + Issues

Use `write_file` to create the file with:
...
[List each with specific page URLs and recommended fix]
...
**After Stage 1, immediately proceed to Stage 2. Do not wait for user confirmation.**

### Stage 2: Category Deep Dives

Use `edit_file` to append before the last line, or read the file and rewrite it with the deep dives appended:
...
### AI Citability ([X]/100)
[Detailed findings, examples, rewrite suggestions]

### Brand Authority ([X]/100)
[Platform presence, mention volume, sentiment]
...
**After Stage 2, immediately proceed to Stage 3.**

### Stage 3: Action Plan + Appendix
...
**After Stage 3, the report is complete. Confirm to the user with the file path.**
