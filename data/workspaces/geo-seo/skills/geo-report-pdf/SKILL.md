...
- The PDF generation script is located at: `workspace/scripts/generate_pdf_report.py`
...
Write the collected audit data to a temporary JSON file in the workspace:

```bash
python -c "import json, sys; json.dump(audit_data, open('workspace/generated/geo-audit-data.json', 'w'), indent=2)"
```

Or use write_file to create `workspace/generated/geo-audit-data.json`.
...
```bash
python workspace/scripts/generate_pdf_report.py workspace/generated/geo-audit-data.json "workspace/reports/GEO-REPORT-[brand].pdf"
```
...
5. **Write JSON to file** — Save to `workspace/generated/geo-audit-data.json` using write_file.

6. **Run the PDF generator**:
   ```bash
   python workspace/scripts/generate_pdf_report.py workspace/generated/geo-audit-data.json "workspace/reports/GEO-REPORT-[brand_name].pdf"
   ```
