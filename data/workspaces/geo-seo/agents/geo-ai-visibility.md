...
- Use web_fetch to retrieve the target URL.
...
1. **YouTube**: Use web_fetch to search `site:youtube.com "brand name"` patterns. Check for official channel presence, video count, and engagement.
...
3. **Wikipedia (CRITICAL — use API check, not just web search)**:
   - **FIRST**, run the Wikipedia API directly via Bash to check definitively:
     ...
     "
     ```
   - **SECOND**, try web_fetch on `https://en.wikipedia.org/wiki/[Brand_Name]` directly to verify.
...
- If web_fetch fails for a platform check, note the failure and do not fabricate results.
